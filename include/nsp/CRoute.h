 ...
 *System.
 * 					 The Waypoints in the route can be added to
 *this class
 * 					 and is stored here. This class stores the
 *pointers to
 * 					 the POI data in the CPOIdatabase. The POIs to
 *the route
 * 					 can also be added to this class and when doing
 *so the
 * 					 corresponding pointers from the CPoiDatabase
 *class are
 * 					 fetched and stored in this class. Also the
 *route data
 * 					 which consists of Waypoints and POIs can be
 *printed here.
 * 					 Additionally this class provides the method to
 *calculate
 * 					 the distance to the nearest POI from a
 *particular location.
 ****************************************************************************/
...
private:
  ...
  CPoiDatabase
      *m_pPoiDatabase; /**< \brief Represents the poi database connection */
  CWpDatabase
      *m_pWpDatabase; /**< \brief Represents the waypoint database connection */
  CRouteList
      m_routeContainer; /**< \brief Represents the container to store the route
                           */
  unsigned int
      m_waypointCount; /**< \brief Represents the waypoint count in the route */
...
public:
  ...
  /**
  * Parameterized constructor to CRoute class - Sets the values in case of valid
  * input, else takes the default values
  * Allocates the dynamic Memory needed to store the Waypoints and Pointers to
  * the POI Database
  * @param unsigned int maxWp   - Maximum Waypoint			[IN] Default
  * Value: 0
  * @param unsigned int maxPoi  - Maximum Point of Interest	[IN] Default
  * Value: 0
  */
  CRoute();
  /**
  ...
  * Deallocates all the dynamic memory allocated for Waypoint objects and for
  * the Pointer to Database objects
  */
  ~CRoute();
  /**
  ...
  * @param std::string namePoi - Name of the Waypoint object to be added to the
  * route [IN]
  * @return None
  */
  ...
  * @param std::string namePoi - Name of the POI object to be added to the route
  * [IN]
  * @param std::string afterWp - Name of the Waypoint object after which the POI
  * is to be added [IN].
  * @return None
  */
  ...
  /**
   * Gets the distance to the nearest POI (from the current location) by
   * traversing through the POIs of the current route
   * @param const CWaypoint& wp - Reference to the Waypoint object [IN]
   * @param CPOI& poi   		  - Reference to the POI object
   * [OUT]
   * @return distance  		  - Distance to the nearest POI
   */
  double getDistanceNextPoi(const CWaypoint &wp, CPOI &poi);
  /**
   * Prints the Waypoints(either in DEGREE or MMSS format) and POIs in the
   * current route
   * @param None
   * @return None
   */
