...
private:
  ...
  std::vector<char> m_deliemiterVec; /**< \brief Represents the container that
                                        holds the delimiters */

  unsigned int max_attributes; /**< \brief Represents the max number of
...
public:
   ...
   * @param unsigned int maxAttributes [IN] - The maximum attributes supported
   * in the text
   * @return None
  */
  ...
  /**
       ...
       * @param std::vector<std::string>& attributeVec [OUT] - vector of strings
   * which holds the result after extraction
       * @return RC_t - the error code after extraction
       */
  RC_t extractAttributes(const std::string &line,
                         std::vector<std::string> &attributeVec);
  /**
       * Prints the content of WP Database
       * @param RC_t& ec[IN] - Reference to the error code to be converted to
   * string
