 ...
 * Initializes all the data members with the values provided or with the default
 * values
 * @param t_poi type 			  - type of the POI object
 * [IN] Default: UNKNOWN
 * @param std::string name 		  - Name of the POI object
 * [IN] Default: EMPTY_STRING
 * @param std::string description - Description of the POI object	[IN]
 * Default: EMPTY_STRING
 * @param double latitude 	 	  - Latitude of the POI object
 * [IN] Default: 0.0
 * @param double longitude 		  - Longitude of the POI object
 * [IN] Default: 0.0
 */
 ...
 * @param std::string& name 		  - Name of the CPOI object
 * [OUT]
 * @param std::string& description 	  - Description of the CPOI object
 * [OUT]
 * @param double& latitude  		  - Latitude value of the CPOI object
 * [OUT]
 * @param double& latitude  		  - Longitude value of the CPOI object
 * [OUT]
 * @return None
 */
 ...
 * @return None
 */
void CPOI::print(void) { std::cout << *this << "\n"; }
/**
 * Overloaded operator<< for POI class. Prints all POI specific data to the
 * given ostream object
 * @param  std::ostream& stream [OUT] - The ostream object where the POI data is
 * to be pushed into
...
std::ostream &operator<<(std::ostream &stream, const CPOI &poi)
{
  ...
  poi.getAllDataByReference(
      name, latitude, longitude, type,
      description); /*Populate the local variables with the object data */
  ...
  stream << name << " on latitude = " << lat_deg << " deg " << lat_mm << " min "
         << lat_ss << " s"
         << " and longitude = " << lon_deg << "deg " << lon_mm << " min "
         << lon_ss << " s" << std::endl;
  /*stream << name << SPACE_CHAR << "on latitude:" << latitude
                    << SPACE_CHAR << "and longitude:" << longitude <<
     std::endl;*/
  return stream;
}
/**
 ...
 */
std::string CPOI::getDescription() const { return m_description; }
/**
 * Gets the type field of the POI object
 * @return string - the poi type
 */
CPOI::t_poi CPOI::getType() const { return m_type; }
