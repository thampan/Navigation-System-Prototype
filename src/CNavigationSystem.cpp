    ...
    : m_route() /* Initialize m_route object */
{
  ...
  CWaypoint wpTable[] = {
      ...
      CWaypoint(52.5166, 13.4050, "Berlin"), CWaypoint(49.8673, 8.6382, "H_da"),
      CWaypoint(32.3680, 4.9036, "Amsterdam"),
      CWaypoint(48.7823, 9.1770200, "Stuttgart"),
      CWaypoint(49.4076, 8.69079, "Heidelberg"),
      CWaypoint(49.49671, 8.47955,
                "Mannheim")}; /* Table which stores the Waypoint data which is
   ...
   * Database */
  CPOI poiDbTable[] = {
      CPOI(CPOI::RESTAURANT, "Mensa HDA", "the best Mensa", 49.8728, 8.6512),
      CPOI(CPOI::TOURISTIC, "Frankfurt", "Rhine River", 48.8738, 6.6712),
      CPOI(CPOI::RESTAURANT, "Sitte", "more expensive but good", 49.6118,
           8.4578),
      CPOI(CPOI::GASSTATION, "Darmstadt", "more expensive but good", 49.8728,
           8.2512),
      CPOI(CPOI::UNIVERSITY, "TU Darmstadt", "Research oriented", 49.8889,
           8.6121),
      CPOI(CPOI::TOURISTIC, "Eberstadt", "Frankenstein Castle!!", 49.2233,
           10.4312),
      CPOI(CPOI::GASSTATION, "Total Tankstelle", "Best gasstation in Berlin",
           52.5200, 8.7766),
      CPOI(CPOI::RESTAURANT, "Radieschen", "Vegan Restaurant", 49.0011, 8.9876),
      CPOI(CPOI::TOURISTIC, "Frankfurt Cathedral", " Roman Catholic Church",
           48.9988, 6.2123)};
  ...
  m_route.connectToPoiDatabase(&m_PoiDatabase); /* Connect Route to the POI
                                                   Database from where POIs can
                                                   be fetched */
 ...
 * Invokes the method which prints the distance to next POI from current
 * position
 * @param  None
 * @return None
 */
...
void CNavigationSystem::printDistanceCurPosNextPoi(void) {
  ...
  if (DEFAULT_STRING !=
      wp.getName()) /*Check if the current position is valid */
  {
    CPOI
        cpoiObj; /* POI object to hold the POI data which would be returned
                                    after searching for the nearest POI in the
                    database */

    double distanceToNextPoi = m_route.getDistanceNextPoi(
        wp, cpoiObj); /*Fetches the distance to the nearest POI */

    if (0 <= distanceToNextPoi) /*Checks if the calculated distance is valid */
    {
      std::cout << "Distance to the next POI = " << distanceToNextPoi
                << std::endl;
...
void CNavigationSystem::TC_addMoreWaypointsThanSpecified() {
  const CWaypoint wpTable[] = {
      ...
      CWaypoint(49.8673, 8.6382, "H_da")}; /* Table which stores the Waypoint
                                              data which is to be added to the
                                              Route */
 ...
 * Expected Result: No crashes or impacts to the waypoint part. System is still
 * stable
 * @param  None
 * @return None
 */
