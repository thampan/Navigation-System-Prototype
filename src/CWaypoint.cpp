 ...
 * 0.0  [IN]
 * @param std::string name - Name of the CWaypoint object, default: EMPTY_STRING
 * [IN]
 */
CWaypoint::CWaypoint(double latitude, double longitude, std::string name)
  ...
  std::cout << "CWaypoint::CWaypoint:name: " << this->m_name
            << ", addr:" << &this->m_name << std::endl;
  std::cout << "CWaypoint::CWaypoint:latitude: " << this->m_latitude
            << ", addr:" << &this->m_latitude << std::endl;
  std::cout << "CWaypoint::CWaypoint:longitude: " << this->m_longitude
            << ", addr:" << &this->m_longitude << std::endl;
#endif
}
/**
...
bool CWaypoint::set(std::string name, double latitude, double longitude) {
  ...
  do {
    if (((MIN_LATITUDE <= latitude) && (MAX_LATITUDE >= latitude)) &&
        ((MIN_LONGITUDE <= longitude) &&
         (MAX_LONGITUDE >=
          longitude))) /* Checks if the latitude and longitude
            ...
            object attributes to default values */
  {
    ...
    std::cout << "WARNING! CWaypoint::set(): Invalid values provided, so "
 ...
 * @return name
 */
std::string CWaypoint::getName() const { return m_name; }
/**
 * Get the attribute 'latitude' of CWaypoint object
 * @param None
 * @return latitude
 */
double CWaypoint::getLatitude() const { return m_latitude; }
/**
 * Get the attribute 'longitude' of CWaypoint object
 * @param None
 * @return longitude
 */
double CWaypoint::getLongitude() const { return m_longitude; }
/**
  ...
  double latitude_1 =
      CONVERT_DEGREE_TO_RADIAN(
          this->getLatitude()); /* Convert the first latitude from degree to
                                   radian format*/
  double longitude_1 =
      CONVERT_DEGREE_TO_RADIAN(this->getLongitude()); /* Convert the first
                                                         longitude from degree
                                                         to radian format*/

  double latitude_2 =
      CONVERT_DEGREE_TO_RADIAN(wp.m_latitude); /* Convert the second latitude
                                                  from degree to radian format*/
  double longitude_2 = CONVERT_DEGREE_TO_RADIAN(
      wp.m_longitude); /* Convert the second longitude from degree to radian
                          format*/

  double distance = EARTH_RADIUS_LENGTH *
                    acos(sin(latitude_1) * sin(latitude_2) +
                         cos(latitude_1) * cos(latitude_2) *
                             cos((longitude_2 - longitude_1))); /* Calculate
                                                                   distance
                                                                   between two
                                                                   Waypoints
                                                                   using the
 ...
 * @param int format - The format in which the CWaypoint object is to be printed
 * - DEGREE or MMSS format [IN]
 * @return None
 */
void CWaypoint::print(int format) { std::cout << (*this) << std::endl; }
/**
...
void CWaypoint::transformLongitude2degmmss(int &deg, int &mm,
  ...
  double decimal =
      this->getLongitude() - deg; /* Get the decimal part in the longitude */
  ...
  double seconds =
      minutes - mm;           /* Get the decimal portion in the minutes data*/
  ss = (seconds * TIME_UNIT); /* To get the second count, again multiply the
                                 decimal portion by 60*/
}
/**
...
std::ostream &operator<<(std::ostream &stream, CWaypoint &wp) {
  ...
  wp.transformLongitude2degmmss(
      lon_deg, lon_mm, lon_ss); /* Transforms the longitude to DEGSS format*/
  wp.transformLatitude2degmmss(
      lat_deg, lat_mm, lat_ss); /* Transforms the latitude to DEGSS format*/

  stream << wp.getName() << " on latitude = " << lat_deg << " deg " << lat_mm
         << " min " << lat_ss << " s"
         << " and longitude = " << lon_deg << "deg " << lon_mm << " min "
         << lon_ss << " s" << std::endl;
