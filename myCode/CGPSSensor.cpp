 ...
 * 					 a CWaypoint object out of these coordinates and return this
 * 					 object when invoked.
 ****************************************************************************/
...
CWaypoint CGPSSensor::getCurrentPosition(void)
{
	...
	if(std::cin.fail()) /* In case the input validation fails */
	{
		...
		std::cout << "ERROR!CGPSSensor::getCurrentPosition(): Invalid Latitude! Hence Defaulting to 0" << std::endl;
		name = DEFAULT_STRING; /* Set the name to EMPTY_STRING which indicates the position is invalid */
	}
	...
	if(std::cin.fail()) /* In case the input validation fails */
	{
		std::cin.clear();/* Clear the error state of the buffer */
		std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); /* Ignore the rest of the line */
		std::cout << "ERROR!CGPSSensor::getCurrentPosition(): Invalid Longitude! Hence Defaulting to 0" << std::endl;
		name = DEFAULT_STRING; /* Set the name to EMPTY_STRING which indicates the position is invalid */
	}
	return CWaypoint(latitude, longitude, name); /*Return CWaypoint object after setting the attributes */
}
