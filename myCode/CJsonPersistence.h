...
#define OBJECT_SEPARATOR_TOKEN ","  /**< \brief Represents Object Separator token */

class CJsonPersistence:public CPersistentStorage
{
	private:

		typedef std::vector<std::string> StringVector;	/**< \brief Represents a vector of strings */

		typedef struct
		{
			std::string errorMsg;	/**< \brief Describes the error message to be displayed */
			bool isFatal;			/**< \brief Describes if the error is fatal or minor */
		}error_data_t; 				/**< \brief Contains the error data */

		typedef enum
		{
			IS_WAITING_FOR_FIRST_TOKEN,					/**< \brief Represents the IS_WAITING_FOR_FIRST_TOKEN state */
			IS_WAITING_FOR_DB_NAME,						/**< \brief Represents the IS_WAITING_FOR_DB_NAME state */
			IS_WAITING_FOR_DB_NAME_SEPARATOR,			/**< \brief Represents the IS_WAITING_FOR_DB_NAME_SEPARATOR state */
			IS_WAITING_FOR_DB_ARRAY_BEGIN,				/**< \brief Represents the IS_WAITING_FOR_DB_ARRAY_BEGIN state */
			IS_WAITING_FOR_DB_OBJ_BEGIN,				/**< \brief Represents the IS_WAITING_FOR_DB_OBJ_BEGIN state */
			IS_WAITING_FOR_DB_OBJ_ATTR_NAME,			/**< \brief Represents the IS_WAITING_FOR_DB_OBJ_ATTR_NAME state */
			IS_WAITING_FOR_DB_OBJ_ATTR_NAME_SEPARATOR,  /**< \brief Represents the IS_WAITING_FOR_DB_OBJ_ATTR_NAME_SEPARATOR state */
			IS_WAITING_FOR_DB_OBJ_ATTR_VALUE,			/**< \brief Represents the IS_WAITING_FOR_DB_OBJ_ATTR_VALUE state */
			IS_WAITING_FOR_DB_OBJ_ATTR_VALUE_SEPARATOR, /**< \brief Represents the IS_WAITING_FOR_DB_OBJ_ATTR_VALUE_SEPARATOR state */
			IS_WAITING_FOR_DB_OBJ_SEPARATOR,			/**< \brief Represents the IS_WAITING_FOR_DB_OBJ_SEPARATOR state */
			IS_WAITING_FOR_DB_ARRAY_SEPARATOR,			/**< \brief Represents the IS_WAITING_FOR_DB_ARRAY_SEPARATOR state */
			IS_WAITING_FOR_DB_OBJ_END,					/**< \brief Represents the IS_WAITING_FOR_DB_OBJ_END state */
			IS_WAITING_FOR_DB_ARRAY_END,				/**< \brief Represents the IS_WAITING_FOR_DB_ARRAY_END state */
			IS_WAITING_FOR_PARSE_END,					/**< \brief Represents the IS_WAITING_FOR_PARSE_END state */
		}parser_state_t; 								/**< \brief Represents the states of the parser state machine */
		...
		}db_type_t;	/**< \brief Represents the states of the types of supported databases */
		...
		typedef struct __data
		{
			...
			std::string description;	/**< \brief Represents the description of the poi */
		...
		typedef struct __stateInfo
		{
			...
			parser_state_t currentState;	/**< \brief Represents the current state of the state machine*/
			db_type_t dbType;				/**< \brief Represents the type of database*/
			std::string parameter;			/**< \brief Represents the type of parameter found in the previous iteration*/
		...
		std::string m_fileName; 			/**< \brief Represents Waypoint File Name (with path)*/
		StringVector jsonFileContents;		/**< \brief Represents a Vector of strings to hold the json file contents */
...
private:
		 ...
		 * @return None
		 */
		void processStateChange(APT::CJsonToken* currentToken, stateInfo_t& stateInfo);
		/**
		 ...
		 */
		bool checkAndExtractParameterName(const db_type_t& db, const std::string& in, std::string& out);
		/**
		 ...
		 *
		 * @param std::map<std::string, DataType>& dataMap: [IN] the Reference to the Map containing the Data contents (Wp Data or POI Data)
		 * @return None
		 */
		...
		{
		    size_t count = 1;
			for (typename std::map<std::string, DataType>::const_iterator itr = dataMap.begin(); itr != dataMap.end(); ++itr, ++count)
			{
				DataType dataType = itr->second;
				formatJsonData(dataType);
				(count == dataMap.size()) ? jsonFileContents.push_back("\t  }") : jsonFileContents.push_back("\t  },");
			}
		}
		/**
		 ...
		 * @param bool isNumber: [IN] if the input is a number, quotes will not be applied to it
		 * @return string - Modified string
		 */
		inline std::string to_json_attributes(std::string parameter, std::string& value, bool isNumber = false)
		{
			std::string result = DOUBLE_TAB_STRING + QUOTE_STRING + parameter + QUOTE_STRING + COLON_STRING;
			if(!isNumber)
			{
		 ...
		 *
		 * @param Database& databaseObj: [IN] the Reference to the Database object - WaypointDb or POIDb
		 * @return None
		 */
		template <typename Database>
		void formatJsonData(Database& databaseObj)
		{
			...
			std::string jsonLatitudeStr   =  to_json_attributes("latitude", latitudeStr, true) + COMA_STRING;
			jsonFileContents.push_back(jsonLatitudeStr);

			std::string longitudeStr      =  type_mapping::to_string(longitude);
			std::string jsonLongitudeStr  =  to_json_attributes("longitude", longitudeStr, true);
			if(poiPtr!=NULL) jsonLongitudeStr += COMA_STRING;
			...
			if(poiPtr)
			{
				...
				{
					std::cout << "ERROR!JSON::writeData() failed! Invalid POI Type!" << std::endl;
				}
				std::string descriptionData =  poiPtr->getDescription();
				std::string jsonDescription = to_json_attributes("description", descriptionData) + COMA_STRING;
