...
#define POI_DESCRIPTION_POS (2)			/**< \brief Represents the position of POI Description */
#define POI_LATITUDE_POS (3)			/**< \brief Represents the position of POI Latitude */
#define POI_LONGITUDE_POS (4)			/**< \brief Represents the position of POI Longitude */

#define WAYPOINT_NAME_POS (0)			/**< \brief Represents the position of Waypoint name */
#define WAYPOINT_LATITUDE_POS (1)		/**< \brief Represents the position of Waypoint latitude */
#define WAYPOINT_LONGITUDE_POS (2)		/**< \brief Represents the position of Waypoint longitude */

/**
 * NameSpace type_mapping stores all the relevant methods of converting from one type to another.
 * There is no storage involved and hence all are simple C-like utility functions. Additionally
 * it performs all necessary validations while converting from one type to another.
 * Also reports the relevant errors which might arise during the type conversions.
 */
...
{
	if((MIN_LATITUDE <= latitude) && (MAX_LATITUDE >= latitude)) /* Checks if the Latitude is within the allowed range */
	{
		return true;
	}
	return false;
...
{
	if((MIN_LONGITUDE <= longitude) && (MAX_LONGITUDE >= longitude))  /* Checks if the Longitude is within the allowed range */
	{
		return true;
	}
	return false;
...
bool convertStringToDouble(const std::string& attr, double& val)
{
	...
	for(; i < attr.length() && (dotCount <= 1); i++)
	{
		...
		/* Validation-1: Allow '-' character only in the beginning to indicate a negative number */
		if(character == '-' && i == 0)
			continue;
	...
	/* Iterator has reached the end of the string => all characters have been validated and fine,
...
errorCode_t convertStringToCPoiType(const std::vector<std::string>& poiAttributeVec, CPOI &poi)
{
	...
	do
	{
		...
		if(!convertStringToLatitude(poiAttributeVec.at(POI_LATITUDE_POS), latitude))
		{
			retVal = RC_INVALID_LATITUDE;
			break;
		}
		/* Validation-4: Check if there is a valid longitude */
		if(!convertStringToLongitude(poiAttributeVec.at(POI_LONGITUDE_POS), longitude))
		{
		...
		if(!validateStringData((description = poiAttributeVec.at(POI_DESCRIPTION_POS))))
		{
...
errorCode_t convertStringToCWaypointType(const std::vector<std::string>&wpAttributeVec, CWaypoint &wp)
{
	...
	do
	{
		...
		if(!convertStringToLatitude(wpAttributeVec.at(WAYPOINT_LATITUDE_POS), latitude))
		{
			retVal = RC_INVALID_LATITUDE;
			break;
		}
		/* Validation-3: Check if there is a valid longitude */
		if(!convertStringToLongitude(wpAttributeVec.at(WAYPOINT_LONGITUDE_POS), longitude))
		{
			retVal = RC_INVALID_LONGITUDE;
			break;
		}
